...
def get_selection_coordinates(img):
...
if __name__ == "__main__":
    ...
    blurred_img = cv.medianBlur(img, 23) # blurred image

    # Get the water mark coordinates
    point1, point2 = get_selection_coordinates(img)
    ...
    cv.createTrackbar('Threshold','Result', threshold, 255, callback)
    ...
    while True:
        ...
        # canny = cv.morphologyEx(canny,cv.MORPH_CLOSE,np.ones((3,3), np.uint8))
